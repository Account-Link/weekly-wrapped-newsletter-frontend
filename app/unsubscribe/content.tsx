...
import DisconnectTiktokRiv from "@/assets/rive/disconnect_tiktok.riv";
...
export default function UnsubscribeClient() {
  ...
  // 重要逻辑：断开连接 loading 使用 Rive 动画，替代 GIF
  const { RiveComponent: DisconnectingRive, rive: disconnectingRive } = useRive(
    {
      src: DisconnectTiktokRiv,
      autoplay: false,
      // stateMachines: "State Machine 1", // 用户要求直接播放 Timeline 1，不再使用状态机
      // 重要逻辑：保持动画等比缩放并居中显示
      layout: new Layout({
        fit: Fit.Contain,
        alignment: Alignment.Center,
      }),
      // 重要逻辑：动画播放结束（OneShot）触发回调，自动跳转到下一状态
      onStop: () => {
  ...
  useEffect(() => {
    if (state !== "disconnecting" || !disconnectingRive) return;

    // 重要逻辑：增加等待机制，确保 Rive 资源完全加载且包含目标动画后再播放
    let rafId: number;
    const waitForReady = () => {
      // 优先查找 "Timeline 1"，如果不存在（可能是资源里命名不同），则回退播放第一个可用动画
      const targetAnim = disconnectingRive.animationNames.includes("Timeline 1")
        ? "Timeline 1"
        : disconnectingRive.animationNames[0];
      ...
      // 播放目标动画
      disconnectingRive.play(targetAnim);
    };

    waitForReady();

    return () => cancelAnimationFrame(rafId);
  }, [state, disconnectingRive]);
  ...
  const handleDisconnect = async () => {
    ...
    // 重要逻辑：请求中状态用于禁用按钮并展示进度
    setIsDisconnectingRequest(true);
  ...
  return (
    <>
            ...
            <div className="h-full w-full flex flex-col justify-end items-center text-center px-[3.4rem]">
              ...
              <div className="w-full flex flex-col items-center justify-center">
                <button
                  ...
                  disabled={!uid || isDisconnectingRequest}
                  className={`block w-[33.4rem] h-[5.2rem] rounded-full text-[1.6rem] font-bold transition-opacity ${
                    isDisconnectingRequest ? "opacity-70" : "hover:opacity-90"
      ...
      <motion.div
        ...
        initial={false}
        animate={{ opacity: state === "disconnecting" ? 1 : 0 }}
        transition={{ duration: 0.3 }}
      >
        <div className="w-[33.5rem] h-[33.5rem]">
          <DisconnectingRive />
        </div>
      </motion.div>
    </>
